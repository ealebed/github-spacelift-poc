...
# For our example we're granting AdministratorAccess, but you can restrict this to whatever you need.
...
data "spacelift_aws_integration_attachment_external_id" "storage-dev-stack" {
...
data "spacelift_aws_integration_attachment_external_id" "storage-prod-stack" {
  integration_id = spacelift_aws_integration.this.id
...
resource "aws_iam_role" "this" {
  ...
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      jsondecode(data.spacelift_aws_integration_attachment_external_id.storage-dev-stack.assume_role_policy_statement),
      jsondecode(data.spacelift_aws_integration_attachment_external_id.storage-prod-stack.assume_role_policy_statement),
    ]
  })
}
